-- Corrigir relatórios duplicados no Instagram
...
ON public.relatorio_analise_instagram (profile, nome_documento, (created_at::date))
WHERE profile IS NOT NULL AND nome_documento IS NOT NULL;

-- Prefeito
CREATE UNIQUE INDEX IF NOT EXISTS idx_unique_relatorio_prefeito 
ON public.relatorio_analise_prefeito (profile, nome_documento, (created_at::date))
WHERE profile IS NOT NULL AND nome_documento IS NOT NULL;

-- Web
CREATE UNIQUE INDEX IF NOT EXISTS idx_unique_relatorio_web 
ON public.relatorio_analise_web (profile, nome_documento, (created_at::date))
WHERE profile IS NOT NULL AND nome_documento IS NOT NULL;

-- Qualitativo
CREATE UNIQUE INDEX IF NOT EXISTS idx_unique_relatorio_qualitativo 
ON public.relatorio_qualitativo (profile, nome_documento, (created_at::date))
WHERE profile IS NOT NULL AND nome_documento IS NOT NULL;